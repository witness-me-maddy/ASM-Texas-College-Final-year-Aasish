...
class ASMScannerService:
    ...
    def _run_tool_or_fallback(self, tool: str, cmd: list[str], host: str) -> tuple[str, ToolRun]:
        ...
        def _success_run(message: str | None = None, return_code: int | None = 0) -> ToolRun:
            return ToolRun(
                report_id="pending",
                tool_name=tool,
                command=command_str,
        ...
        if not shutil.which(binary):
            fallback_output = self._internal_tool_fallback(tool, host)
            if fallback_output:
                return fallback_output, _success_run(message=f"internal fallback used because binary {binary} was unavailable")
            run = ToolRun(
                report_id="pending",
                tool_name=tool,
                command=command_str,
        ...
        try:
            ...
            if completed.returncode == 0 and output:
                return output, _success_run(return_code=completed.returncode)

            fallback_output = self._internal_tool_fallback(tool, host)
            if fallback_output:
                msg = f"internal fallback used after command exit={completed.returncode}"
                return fallback_output, _success_run(message=msg, return_code=completed.returncode)

            status = ToolRunStatus.error if completed.returncode != 0 else ToolRunStatus.empty
            message = f"exit code {completed.returncode}" if completed.returncode != 0 else "command returned no output"
            run = ToolRun(
                report_id="pending",
                tool_name=tool,
                command=command_str,
            ...
            return (f"TOOL_ERROR:{tool}:exit_{completed.returncode}" if status == ToolRunStatus.error else f"TOOL_EMPTY:{tool}"), run
        except Exception as exc:
            fallback_output = self._internal_tool_fallback(tool, host)
            if fallback_output:
                return fallback_output, _success_run(message=f"internal fallback used after exception {exc.__class__.__name__}")
    ...
    def _internal_tool_fallback(self, tool: str, host: str) -> str:
        if tool in {"Nmap", "Masscan", "Naabu"}:
            ports = self._builtin_port_probe(host)
            return "\n".join(f"{p}/tcp open {svc}" for p, svc in ports)
        if tool in {"Subfinder", "Assetfinder", "Amass"}:
            return "\n".join(self._builtin_subdomain_probe(host))
        if tool == "httpx":
            return self._builtin_httpx_probe(host)
        if tool == "Wafw00f":
            return self._builtin_waf_probe(host)
        if tool == "Nikto":
            return self._builtin_nikto_probe(host)
        if tool == "Nuclei":
            return self._builtin_nuclei_probe(host)
        return ""

    @staticmethod
    def _builtin_port_probe(host: str) -> list[tuple[int, str]]:
        common_ports = [80, 443, 22, 25, 53, 110, 143, 3306, 5432, 6379, 8080, 8443]
        services = {
            80: "http",
            443: "https",
            22: "ssh",
        ...
        for port in common_ports:
            try:
                with socket.create_connection((host, port), timeout=1.0):
                    found.append((port, services.get(port, "unknown")))
            except Exception:
                continue
        return found[:20]

    @staticmethod
    def _builtin_subdomain_probe(host: str) -> list[str]:
        prefixes = ["www", "api", "dev", "staging", "mail", "app"]
        found: list[str] = []
    ...
    @staticmethod
    def _builtin_httpx_probe(host: str) -> str:
        ...
        for url in urls:
            try:
                ...
                rows.append(f"{resp.url} [{resp.status_code}] title={title} server={server}")
                break
            except Exception:
                continue
        return "\n".join(rows)
    ...
    def _builtin_waf_probe(host: str) -> str:
        ...
        for needle, label in waf_map.items():
            if needle in headers:
                return f"WAF detected: {label}"
        return "No WAF signature identified"

    @staticmethod
    def _builtin_nikto_probe(host: str) -> str:
        try:
            with httpx.Client(timeout=6.0, follow_redirects=True, verify=False) as client:
                resp = client.get(f"https://{host}")
        except Exception:
            return "Nikto fallback could not establish HTTPS connection"

        findings: list[str] = []
        header_keys = {k.lower() for k in resp.headers.keys()}
        if "x-powered-by" in header_keys:
            findings.append("Header disclosure: X-Powered-By is exposed")
        if "server" in header_keys:
            findings.append("Server banner is exposed")
        if "set-cookie" in header_keys and "secure" not in resp.headers.get("set-cookie", "").lower():
            findings.append("Cookie missing Secure attribute")
        return "\n".join(findings)

    @staticmethod
    def _builtin_nuclei_probe(host: str) -> str:
        try:
            with httpx.Client(timeout=6.0, follow_redirects=True, verify=False) as client:
                resp = client.get(f"https://{host}")
        except Exception:
        ...
        hits: list[str] = []
        if "index of /" in body:
            hits.append("[medium] exposed-directory-listing")
        if "trace" in resp.headers.get("allow", "").lower():
            hits.append("[medium] risky-http-method-enabled")
        return "\n".join(hits)
